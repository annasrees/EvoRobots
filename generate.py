...
def Create_world():
    ...
    # pyrosim.Send_Cube(name="Box", pos=[x + 4, y + 4, z] , size=[length, width, height], mass=10.0) #position changed so out of way; module E step 20   
    pyrosim.End()
...
def Generate_Brain():
    ...
    # pyrosim.Send_Synapse( sourceNeuronName = 0 , targetNeuronName = 4 , weight = 0.5 )
    for s in range(0,3): #iterating over sensors
        for m in range(3, 5): #iterating over motors\
            pyrosim.Send_Synapse(sourceNeuronName= s, targetNeuronName= m, weight = random.random())
